...
- [第五卷：实验军火](#第五卷实验军火)
  - [第9章 · TTV炸弹制造（威力公式全解析）](#第9章--ttv炸弹制造威力公式全解析)
    - [9.1 基础概念与爆炸全链路](#91-基础概念与爆炸全链路)
    - [9.2 爆炸公式完整解析（power → 实际爆炸范围）](#92-爆炸公式完整解析power--实际爆炸范围)
    - [9.3 气体选择与配比（含温度与威力关系全解）](#93-气体选择与配比含温度与威力关系全解)
    - [9.4 清洗](#94-清洗)
    - [9.5 气罐加压与预热（威力关键两步）](#95-气罐加压与预热威力关键两步)
    - [9.6 TTV组装](#96-ttv组装)
    - [9.7 运输与安全](#97-运输与安全)
    - [9.8 引爆流程与威力测算](#98-引爆流程与威力测算)
    - [9.9 进阶技术](#99-进阶技术)
    - [9.10 哑弹排查（修订版）](#910-哑弹排查修订版)
...
**代码**: `transfer_valve.dm` (435行), `tanks/tanks.dm` (爆炸核心), `gasmixtures/reactions.dm` (燃烧反应), `controllers/subsystem/explosions.dm` (威力换算)

### 9.1 基础概念与爆炸全链路

**TTV = Transfer Tank Valve**: 一个阀门+两个气罐+触发装置=炸弹。TTV 本体**不爆炸**——它只负责把两罐气体合并；爆炸发生在**气罐碎裂**时。

**核心时序（实测）**：
```
气罐从生成起就每 2 秒 process 一次（SSobj tick）：
  process() = ① air_contents.react(src)  ← 罐内气体燃烧/反应
              ② handle_tolerances()      ← 压力/温度伤害判定
TTV 开阀 toggle_valve() → merge_gases()：
  仅合并气体（体积相加 140L、摩尔数相加、温度按热容加权），不触发反应
→ 下一个 process tick（≤2 秒）：
  温度够 → 燃烧 → 压力暴涨 → handle_tolerances 判定 → 碎裂 → 爆炸
```

> ⚠️ **最重要的事实：TTV 是"单 tick 判定"的瞬时装置**。开阀后最多 **1 个 process tick（2 秒）** 内完成"燃烧→碎裂→爆炸"全流程——若合并后温度/压力不足，燃烧不发生、压力不涨，罐子**永远不会炸**（哑弹）。实战中开阀瞬间就出结果：**炸就是炸，不炸就是不炸**，不存在"等一会儿再炸"。

**爆炸全链路（按代码执行顺序）**：
```
1. 开阀 toggle_valve() → merge_gases()：两罐气体合入 tank_two（默认）
   - 体积相加：70L + 70L = 140L
   - 摩尔数相加；温度按热容加权平均（不点燃！）
   - 合并后压力 P = nRT/V（加权平均，通常比高罐低）
2. 下一个 process tick（≤2 秒）：
   ① air_contents.react(src)  ← 若 T ≥ 373.15K 且配比正确 → 燃烧放热 → T↑ → P↑
   ② handle_tolerances()：
      - 温度 ≥ TANK_MELT_TEMPERATURE (1,000,000K) → 按 (T-1e6)/T 每 tick 受 BURN 伤害
      - 压力 ≥ TANK_LEAK_PRESSURE (3,040kPa) → 每 tick 受 max_integrity×(P-3040)/(3546-3040) 伤害
3. 判定结果（同一 tick 内）：
   - 合并后（含燃烧）P ≥ 3,546kPa → 一次伤害 ≥ max_integrity → 罐体当场碎裂
   - 碎裂时 P > 4,053kPa → 再 react 一轮 → power 计算 → dyn_explosion 爆炸
   - 碎裂时 P ≤ 4,053kPa → 只裂不漏，不爆炸
   - 合并后 P < 3,040kPa → 不受伤 → 永不碎裂 → 哑弹
```

**核心常量**（全部实测自 `atmos_piping.dm`，= atm × 101.325）：

| 常量 | 值 | 含义 |
|---|---|---|
| `TANK_LEAK_PRESSURE` | **3,040 kPa** (30atm) | 超过开始漏气 + 每 tick 受伤起点 |
| `TANK_RUPTURE_PRESSURE` | **3,546 kPa** (35atm) | 受伤比例达 100%——**一次 tick 即碎裂**的临界 |
| `TANK_FRAGMENT_PRESSURE` | **4,053 kPa** (40atm) | 碎裂时**是否爆炸**的阈值 |
| `TANK_FRAGMENT_SCALE` | **8,511 kPa** (84atm) | power 缩放常数 |
| `TANK_MELT_TEMPERATURE` | **1,000,000 K** | 温度熔毁阈值（热炸路径） |

> ⚠️ **关键澄清**：4,053kPa 是"碎裂后是否爆炸"的判定线，不是"到了就炸"的压力。真正决定碎裂的是 3,546kPa（一次 tick 伤害归零）。且全部判定在开阀后 **1 个 tick 内**完成——没有累积多 tick 的"延迟爆炸"过程。

### 9.2 爆炸公式完整解析（power → 实际爆炸范围）

**第一步：power 计算**（tanks.dm L402）
```dm
power = (air_contents.volume × (pressure - TANK_FRAGMENT_PRESSURE)) / TANK_FRAGMENT_SCALE
      = (体积L × (碎裂瞬间压力kPa - 4,053)) / 8,511
```
- 体积 = 合并后体积（双 70L 罐 = **140L**；大罐 TTV 可更高）
- 压力 = **碎裂瞬间**的压力（含最后一轮 react 燃烧的贡献）——这就是为什么反应剧烈的气体威力爆炸性增长

**第二步：power → 爆炸范围**（explosions.dm L186-191，文档原缺失！）
```dm
range = round((2 × power) ** GLOB.DYN_EX_SCALE)   // DYN_EX_SCALE = 0.5 → range = round(√(2×power))
devastation_range = round(range × 0.25)   // 毁灭级（拆墙）
heavy_impact_range = round(range × 0.5)   // 重冲击（重伤）
light_impact_range = round(range)         // 轻冲击（全范围）
flash_range = 1.5 × range                 // 闪光致盲范围
```

**第三步：bombcap 上限**（explosions.dm L334-339，TTV 用 `ignorecap=FALSE`）
| 档位 | 服务器上限 |
|---|---|
| devastation | ≤ 3 格 |
| heavy | ≤ 7 格 |
...
> ⚠️ **重要**：TTV 爆炸**受 bombcap 限制**（不像管理员爆炸 ignorecap=TRUE）。power 无限堆高没有意义——超过上限后只增加闪光，不增加破坏半径。**140L 罐 × 高压气体通常轻松顶满 light=14 格上限**，追求的是"顶满上限所需的最小配比"。

**实用对照表**（双 70L 罐 = 140L，推回所需碎裂压力）：

| 目标 | 所需 range | 所需 power | 所需碎裂压力 |
|---|---|---|---|
| light 5 格 | 5 | 12.5 | 4,813 kPa |
| light 10 格 | 10 | 50 | 7,093 kPa |
| light 14 格（上限） | 14 | 98 | 10,006 kPa |
| heavy 7 格（上限） | 14 | 98 | 10,006 kPa |
| dev 3 格（上限） | 12 | 72 | 8,426 kPa |

> 结论：碎裂压力 ≥ **10,000 kPa** 即顶满全部上限。这就是为什么 TTV 炸弹追求"反应后压力破万"而非无限加压。

### 9.3 气体选择与配比（含温度与威力关系全解）

**燃烧反应共性与门槛**（reactions.dm）：

| 反应 | 最低点燃温度 | 每 mol 燃料放热 | 氧化剂 | 化学计量 | 产物 |
|---|---|---|---|---|---|
| Plasma 燃烧 | **373.15K (100°C)** | **3,000,000 J** | O₂ | O₂:Plasma≈1.4:1（随温度降） | CO₂(75%)+H₂O(25%)，超饱和产氚 |
| H₂ 燃烧 | **373.15K (100°C)** | **2,800,000 J** | O₂ | H₂:O₂=2:1 | H₂O 水蒸气 |
| Tritium 燃烧 | **373.15K (100°C)** | **2,800,000 J** | O₂ | 同 H₂ | H₂O + 辐射脉冲 |

> ⚠️ **全文档最重要的机制**：所有燃烧都要 ≥ **373.15K (100°C)** 才点燃（`FIRE_MINIMUM_TEMPERATURE_TO_EXIST = 100 + T0C`）。**但 373.15K 只是"点火下限"，不是"够用温度"**——详见下方温度曲线。

**温度如何决定威力（正反馈链 + 直接压力贡献）**：
```
① 燃烧速率正反馈（Plasma 专属，H₂/氚无此曲线）：
   temperature_scale = (T - 373.15) / (1643.15 - 373.15)
   T = 373.15K  → scale = 0（速率≈0，等于没烧！）
   T = 1000K   → scale ≈ 0.49（约一半速率）
   T = 1643.15K → scale = 1 封顶（全速燃烧）★ 全速线
   T > 1643.15K → 保持全速
   每 tick 烧等离子 = (plasma_moles/9) × scale

② 温度直接线性放大压力（PV=nRT，所有气体通用！）：
   P = nRT/V，密封罐定容加热：T×17 (293→5000K) → 压力×17
   例：70L 罐 114.9mol 等离子，293K 时 P=4,000kPa；
       加热到 5,000K → P = 114.9×8.31×5000/70 ≈ 68,000kPa
       → 不靠燃烧，光加热压力就已远超 4,053kPa 碎裂线！

③ 两者叠加：高温 → 压力本身就高 + 燃烧全速 → 放热再推压力 → 暴涨
```

> 🔥 **为什么实战必须把等离子加热到几千 K（而不是"100°C 就够"）**：
> 1. **100°C 时 scale≈0，等离子几乎不烧**——而 TTV 只有开阀后**第一个 process tick** 一次燃烧机会：这个 tick 内压力推不过 3,546kPa 碎裂线，罐子就永远不炸（哑弹）。100°C 的等离子在这个 tick 里基本不反应，必哑；
> 2. **1643.15K 以上才全速燃烧**（scale=1），每 tick 烧 1/9 等离子、放热 3e6 J/mol——第一个 tick 内压力即可暴涨破碎裂线；
> 3. **温度本身直接贡献压力**（PV=nRT）：几千 K 的罐开阀合并后，即使不反应压力也远超 4,053kPa，第一个 tick 直接触发碎裂爆炸；4000kPa 常温罐合并后可能只有 ~2000kPa，远不够炸。
> 4. H₂/氚炸弹**不需要**几千 K（无温度加速、过 100°C 即全速），但预热到 500~1000K 仍有收益：初始压力更高、开阀后第一个 tick 更稳超阈值。

**热容是隐藏变量**（决定同样放热能升多少度）：温升 ΔT = 放热 J ÷ 混合气热容；热容 = Σ(摩尔数 × 比热容)。

| 气体 | 比热容 J/mol·K | 对炸弹的意义 |
|---|---|---|
| Plasma | **200** | 比热极高→同样热量温升慢（但单mol放热最高 3e6） |
| O₂ | 20 | 氧化剂，比热低 |
| H₂ | 15 | 比热低→温升快，放热 2.8e6/mol |
| Tritium | **10** | 比热最低→**温升最快** + 辐射脉冲 |
| CO₂ / H₂O（产物） | 30 / 40 | 产物比热中等，继续升温 |
| Hypernob | **2000** | 巨型热容→掺入即压温（抑反应原理） |

**结论（指导实战）**：
- **Tritium 炸弹** = 威力天花板：比热 10 温升最快 + 2.8e6 放热 + 辐射脉冲，少量氚即可破万 kPa。
- **H₂ 炸弹** = 性价比之王：比热 15、2.8e6 放热，配 2:1 氧气，预热后轻松顶满。
- **Plasma 炸弹** = 经典但需更多量：单mol放热最高但比热 200 拖累温升，需要更高初始压力或更多摩尔数。
- **混合 Plasma+H₂** = 用 H₂ 低比热快速拉温，Plasma 高放热补能量，爆燃更猛。

**正确配比计算示例**（修正原文档数量级错误）：

```
理想气体方程: n = PV/RT，R = 8.31，T 取常温 293.15K
H₂ 罐 70L / 4,000kPa: n = 4000×70/(8.31×293.15) ≈ 114.9 mol   ← 原文档"1,120mol"错误（×10）
O₂ 罐 70L / 2,000kPa: n = 2000×70/(8.31×293.15) ≈ 57.5 mol
H₂:O₂ = 114.9:57.5 = 2:1 ✓ 化学计量正确
理论放热: 114.9 mol × 2.8e6 = 3.22e8 J   ← 原文档"3.14e9 J"错误（×10）
若温升 3.22e8 J 全用于 H₂O 产物(比热40): ΔT ≈ 3.22e8/(114.9×40) ≈ 70,000K → 压力爆表破万 kPa → 顶满上限
```

### 9.4 清洗

**为什么要清洗**: 残留气体稀释燃料/误反应/哑弹（尤其 Freon 抑燃、Hypernob 压温）
...
**预热（决定成败的关键步骤）** ★:
- **目标**：Plasma 炸弹 → 加热到 **≥1,643K（全速线）**，实战常用 **2,000~5,000K**（超物质引擎/热排气直接取热等离子，或先小规模燃烧自加热）；H₂/氚炸弹 → **≥373.15K 即可全速**，但预热到 500~1,000K 可让初始压力更高、开阀后更快超阈值。
- **判定**：开阀后若温度 <373.15K → 永不燃烧 → 哑弹；等离子若只在 373~1,000K → 开阀后第一个 tick 燃烧极慢（scale 0~0.49），压力推不过碎裂线 → 必哑，实战不可用（TTV 只有一次 tick 机会，不是开阔空间可慢慢烧）。
- **手段**：① 从超物质引擎/热等离子管道直接灌热等离子（实战主流）；② 先充入少量已点燃的高温气体；③ 用热源（焊接枪/加热器）烤罐（慢，只适合 H₂ 弹）。
- **温度即威力**：别忘了 PV=nRT——同样摩尔数，5,000K 罐的初始压力是 293K 罐的 **17 倍**，光温度就把罐压推到碎裂线以上。
...
```
1. 开阀(toggle_valve)→merge_gases：气体合入tank_two，体积相加(140L)，温度热容加权
2. 触发装置激活→process_activation()
3. 下一个 process tick（≤2 秒）：
   ① air_contents.react(src) ← 燃烧（若 T≥373.15K 且配比正确）→ 温度/压力暴涨
   ② handle_tolerances()：
      P ≥ 3,040 → 受伤；P ≥ 3,546 → 一次 tick 伤害≥max_integrity → 碎裂
4. 碎裂判定（同一 tick）：
   if P > 4,053:
     air_contents.react(src) ← 最后一轮燃烧! 压力冲峰值
     power = (140L × (P-4,053)) / 8,511
     range = round(√(2×power))
     dev=round(range×0.25) / heavy=round(range×0.5) / light=round(range)
     flash = 1.5×range
     dyn_explosion(..., ignorecap=FALSE) → 受 dev≤3/heavy≤7/light≤14 封顶
```

**去抖动**: `toggle=FALSE` (0.5秒), 防重复触发

**单 tick 判定表（开阀后 ≤2 秒出结果，无延迟累积）**:

| 开阀合并后状态（含燃烧） | 结果 |
|---|---|
| P < 3,040kPa | 不受伤 → 永不碎裂 → **哑弹** |
| 3,040 ≤ P < 3,546kPa | 每 tick 受伤但 <100% → 可能漏气，**通常不炸** |
| 3,546 ≤ P ≤ 4,053kPa | **当场碎裂**，但 P≤4,053 → **只裂不炸**（漏气） |
| P > 4,053kPa | **当场碎裂 + 爆炸**（威力按 power 公式） |

> **温度视角**：燃烧只发生在开阀后**第一个** process tick（因为要么那 tick 就碎裂爆炸，要么压力不足罐子停止激动状态）。几千 K 热等离子罐开阀合并后压力直接破万（PV=nRT），第一个 tick 必炸；常温罐温度<373.15K 燃烧不发生，压力停留在合并平均值（通常<3,040），永成哑弹。

### 9.9 进阶技术

**混合燃料**: 单罐预先混合Plasma+H₂ → H₂ 低比热快速拉温 + Plasma 高放热补能，叠加放热

**Tritium脏弹**: 附带辐射脉冲，范围 = min(√燃烧Tr摩尔 / 0.5, 20格)，阈值 0.3——辐射范围独立于爆炸范围，**超远距离辐射杀伤**

**温度炸弹（纯热炸）**: 罐内气体加热到 ≥1,000,000K（TANK_MELT_TEMPERATURE）→ 每 tick 受 BURN 伤害熔毁，碎裂时同样触发 power 公式——高温气体本身就能炸，无需燃烧反应（适合 BZ/超物质等高热源）
...
| 开阀温度<100°C不燃 | **没预热！燃烧需≥373.15K** |
| 等离子炸弹烧得极慢/哑弹 | **预热只有 373~1,000K（scale<0.5）**——需 ≥1,643K 全速线，实战 2,000~5,000K |
| 漏气不爆炸 | 压力>3,040但<4,053，或碎裂压力不足（受伤未耗尽） |
| 威力小 | 配比不对/气体不足/温度不够（常温罐压力爬升慢）/未顶满bombcap |
| 完全没反应 | 残留Freon/温度不够/Hypernob |
| 触发没反应 | 频率不对/电池没电 |
