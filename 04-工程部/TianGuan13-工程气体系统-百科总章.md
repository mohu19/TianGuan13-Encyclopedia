...
**爆炸全链路（按代码执行顺序）**：
```
1. 开阀 toggle_valve() → merge_gases()：两罐气体合入 tank_two（默认），体积相加（70L+70L=140L），压力取平均
2. 气罐每 tick 检查 handle_tolerances()：
   - 温度 ≥ TANK_MELT_TEMPERATURE (1,000,000K) → 按 (T-1e6)/T 比例每 tick 受 BURN 伤害
   - 压力 ≥ TANK_LEAK_PRESSURE (3,040kPa) → 按 (P-3040)/(3546-3040) 比例每 tick 受 BRUTE 伤害
3. 罐体 integrity 耗尽 → atom_destruction()：
   - 若 pressure > TANK_FRAGMENT_PRESSURE (4,053kPa)：
     air_contents.react(src)   ← 碎裂前再反应一轮！温度/压力再冲高
     power = (体积 × (pressure - 4,053)) / 8,511
...
| `TANK_RUPTURE_PRESSURE` | **3,546 kPa** (35atm) | 受伤比例达到 100% 的压力点 |
| `TANK_FRAGMENT_PRESSURE` | **4,053 kPa** (40atm) | **碎裂时判定是否爆炸的阈值** |
| `TANK_FRAGMENT_SCALE` | **8,511 kPa** (84atm) | power 缩放常数 |
| `TANK_MELT_TEMPERATURE` | **1,000,000 K** | 温度熔毁阈值（热炸路径） |

> ⚠️ **关键澄清**：`TANK_FRAGMENT_PRESSURE` 不是"压力一到就炸"。气罐先**累积受伤**（泄漏 3,040kPa 起每 tick 受伤，压力越高伤越快），integrity 耗尽才碎裂；碎裂瞬间**再燃烧一轮**（react）把压力推到峰值，此时 >4,053kPa 才产生爆炸。所以**从开阀到爆炸有延迟**（高压快、低压慢甚至不炸）。
...
> 1. **100°C 时 scale≈0，等离子几乎不烧**——正反馈启动极慢，罐内压力爬升以分钟计，大概率在炸之前先漏气/被环境降温，变成哑弹或小威力气浪；
> 2. **1643.15K 以上才全速燃烧**（scale=1），每 tick 烧 1/9 等离子、放热 3e6 J/mol，压力秒级暴涨；
> 3. **温度本身直接贡献压力**（PV=nRT）：几千 K 的罐开阀合并后，即使不反应压力也远超 4,053kPa，直接触发碎裂爆炸；4000kPa 常温罐合并后可能只有 ~2000kPa，远不够炸。
> 4. H₂/氚炸弹**不需要**几千 K（无温度加速、过 100°C 即全速），但预热到 500~1000K 仍有收益：初始压力更高、开阀后更快超阈值。
...
- **判定**：开阀后若温度 <373.15K → 永不燃烧 → 哑弹；等离子若只在 373~1,000K → 燃烧极慢（scale 0~0.49），压力爬升以分钟计，实战几乎不可用。
...
```
1. 开阀(toggle_valve)→merge_gases：气体合入tank_two，体积相加(140L)，压力平均
2. 触发装置激活→process_activation()
3. 气罐每tick累积受伤（P≥3040kPa起）：
   damage = integrity × (P-3040)/(3546-3040) / tick
4. integrity耗尽→碎裂判定：
   if P > 4,053:
...
**引爆延迟估算**（从开阀到爆炸）:
- 压力 3,040~3,546kPa：受伤慢，可能只漏气不炸（哑弹）
- 压力 4,000~6,000kPa：数秒内碎裂爆炸
- 压力 >10,000kPa：几乎瞬间碎裂（1~2 tick），适合远程信号器引爆
- **温度视角**：上述压力是"温度+燃烧"的合力结果——几千 K 热等离子罐开阀合并后压力直接破万（PV=nRT），无需等燃烧即达瞬间碎裂档；常温罐必须先等燃烧把压力推上去，延迟以秒~分钟计。
