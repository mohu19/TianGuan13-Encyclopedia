...
> ⚠️ **全文档最重要的机制**：所有燃烧都要 ≥ **373.15K (100°C)** 才点燃（`FIRE_MINIMUM_TEMPERATURE_TO_EXIST = 100 + T0C`）。**常温两罐气体开阀根本不反应**——必须先用热源/预热把罐内气体加热到 100°C 以上（或用已燃烧的高温气体）。这就是"预热"环节存在的根本原因，也是"哑弹"最常见原因。

**温度如何决定威力（正反馈链）**：
```
温度 T ≥ 373.15K → 燃烧开始
temperature_scale = (T - 373.15) / (1643.15 - 373.15)   // 373~1643K 线性，≥1643K 封顶=1
每 tick 燃烧率 ∝ temperature_scale                         // 越热烧得越快
燃烧放热 → 温度再升 → 燃烧率再升 → 压力暴涨
```
...
**预热（常被忽略的必需步骤）** ★:
- 燃烧需 ≥373.15K。预热手段：① 用热源（焊接枪/加热器）烤罐；② 先充入少量已点燃的高温气体；③ 从热等离子管道直接灌热等离子。
- **判定**：开阀后若温度 <373.15K → 永不燃烧 → 哑弹。预热到 **500~1,000K** 可让 temperature_scale 明显 >0，燃烧率不再趋近于零。
