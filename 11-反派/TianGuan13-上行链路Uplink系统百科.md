# TianGuan13 · 上行链路 Uplink 系统百科

> **项目**: TianGuan13 (Nova Sector 分支)
> **源码**: `code/modules/uplink/`（20 文件 **3,746 行**）+ `code/modules/antagonists/traitor/uplink_handler.dm`（138 行）
> **共用阵营**: **叛徒/核弹队/间谍/承包商**等辛迪加系反派共用（UPLINK_TRAITORS 标志授予）
> **一句话**: 辛迪加官方购物终端——用 **TC 电信水晶**购买 395 件装备（21 分类），从肥皂到机甲全覆盖。
...
| 角色限定 | 部分物品仅特定角色可买 |
| 物种限定 | 部分物品仅特定物种可买 |
| population 要求 | 部分物品需玩家人数达标 |
| 共享库存 | 套装/盈余各 1 库存（shared stock）|
| 购买锁定 | shop_locked（被 EMP/黑客可锁）|
...
| 反派 | Uplink 差异 |
|---|---|
| **叛徒 Traitor** | 标准 Uplink（TC+声誉），任务目标绑定 |
| **核弹队 Nukeop** | 团队 Uplink（队长折扣 5+10 限量），Uplink 物品在飞船 |
| **间谍 Spy** | 伪装 Uplink（PDA/笔/收音机），赏金任务奖励 Uplink 物品 |
| **承包商 Contractor** | 合同制 Uplink（目标+赎金+投放点）|
...
*本文档数值全部实测自 `code/modules/uplink/`（20 文件 3,746 行）源码，无推测。*
