...
## 五、假警报机制（Advisory Deception）

> **源码**: `code/controllers/subsystem/dynamic/dynamic.dm` L306-318（`get_advisory_report()`）
> **核心**：管理层在中央指挥公告里看到的 **Advisory Level 并不一定反映实际威胁等级**——系统有约 25% 概率给出"假警报"。

### 5.1 告警生成逻辑

```
get_advisory_report():
    shown_tier = current_tier.tier          // 实际等级
    if prob(10):                             // ① 10% 完全随机
        shown_tier = pick(4 个等级中非当前的一个)   // 可能跳好几级
    else if prob(15):                        // ② 15% 相邻偏差
        shown_tier = clamp(current_tier ± 1) // 虚报一档
    → 用 shown_tier 的 advisory_report 文案
```
...
|---|---|---|
| 显示真实等级 | **~76.5%** | 告警与实际一致 |
| **完全随机另一等级** | **10%** | 可能显示午夜太阳但实际绿星（或反之） |
| **相邻 ±1 级** | **~13.5%** | 低报一档或高报一档 |

> 计算：10% 完全随机 + 剩余 90% 中 15% ±1 = 10% + 13.5% = **23.5% 假警报**；真实显示 76.5%。

### 5.3 典型"假警报"场景

- **虚高（假红）**：实际绿星/黄星，显示红星/黑色轨道——管理层紧张备战，实际没几个反派
- **虚低（假绿）**：实际黑色轨道/午夜太阳，显示绿星/黄星——玩家放松警惕，实际满配反派
- **完全错位**：10% 跳级——绿星显示午夜太阳（最离谱）

### 5.4 判断真实威胁的方法

| 方法 | 说明 |
|---|---|
| **看实际反派** | 假警报只影响**文字播报**，不影响实际规则集生成——实际刷出的反派最准 |
| **NOVA 目标报告** | NOVA 的 `send_trait_report` 会在 Central Command Status Summary 附动态报告，但同样用 get_advisory_report()（同源假警报） |
| **开场哨站** | greenshift 判断（绿星 + 无目标 = 强制绿星局，NOVA 逻辑）——但同样可能被假警报掩盖 |

---

## 六、config 覆盖（dynamic_config）
